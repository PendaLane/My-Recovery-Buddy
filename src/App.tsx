  ...
  const Sidebar = () => (
    <nav className={mobile ? "fixed bottom-0 w-full bg-white border-t border-[#e5cfe0] flex p-2 z-50 overflow-x-auto gap-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]" : "w-64 bg-[#f8e6f2] border-r border-[#e5cfe0] flex flex-col p-4 h-full shrink-0 overflow-y-auto"}>
      ...
      {[
        ...
        <button key={i.id} onClick={() => setView(i.id as View)} className={`flex items-center gap-3 p-3 rounded-lg text-sm mb-1 whitespace-nowrap transition-all ${view === i.id ? 'bg-[#7A0050] text-white shadow-md' : 'text-[#7A0050] hover:bg-white hover:shadow-sm'}`}>
          <i.icon size={mobile ? 18 : 20} /> {!mobile && i.label}
        </button>
      ))}
      ...
      <div className="mt-auto pt-4 border-t border-[#e5cfe0]/50 flex flex-col gap-2">
        ...
        <button onClick={() => setView('HELP')} className="flex items-center gap-3 p-3 text-sm text-red-600 hover:bg-red-50 rounded-lg whitespace-nowrap font-medium">
            <AlertCircle size={20} /> {!mobile && "Help & Crisis"}
        </button>
      </div>
    </nav>
  );
